...
class CommandMessage {
	...
	parseArgs() {
		switch(this.command.argsType) {
			...
			default:
				throw new RangeError(`Inválido argsType "${this.argsType}".`);
		}
	}
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.reply(`O comando \`${this.command.name}\` só pode ser usado em um canal de servidor.`);
		}

		if(this.command.nsfw && !this.message.channel.nsfw) {
			this.client.emit('commandBlocked', this, 'nsfw');
			return this.reply(`O comando \`${this.command.name}\` só pode ser usando em canais NSFW.`);
		}
		...
		if(this.message.channel.type === 'text' && this.command.clientPermissions) {
			...
			if(missing.length > 0) {
				...
				return this.reply(oneLine`
					Eu precido das seguintes permissões para o comando \`${this.command.name}\` funcionar:
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			return this.reply(
				`Você não pode usar o comando \`${this.command.name}\` novamente por ${remaining.toFixed(1)} segundos.`
			);
		}
		...
		try {
			...
			if(!(retVal instanceof discord.Message || retVal instanceof Array || retVal === null || retVal === undefined)) {
				...
				O comando ${this.command.name} run () resolvido com um tipo desconhecido
			...
			if(err instanceof FriendlyError) {
				...
				Por favor entre em contato com a dona do bot ${ownerList || ''}${invite ? ` neste servidor: ${invite}` : '.'}
				`);
			}
		}
	}
